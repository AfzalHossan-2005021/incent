...
def hierarchical_pairwise_align(
    ...
    **kwargs
    ...
    idx_A, idx_B, dist_A, dist_B, initial_idx_A, initial_idx_B, matched_cluster_pairs = extract_continuous_macro_section(
        sliceA,
        sliceB,
        labelsA,
        labelsB,
    ...
    if len(idx_A) > 0 and len(idx_B) > 0:
        ...
        selected_cluster_pairs = list(matched_cluster_pairs)
        if not selected_cluster_pairs:
            selected_cluster_pairs = [
                (cA, cB)
                for cA in range(Pi_cluster.shape[0])
                for cB in range(Pi_cluster.shape[1])
                if Pi_cluster[cA, cB] > 0
            ]

        for cA, cB in selected_cluster_pairs:
            mass = Pi_cluster[cA, cB]
            if mass > 0:
                # Find cells in the core that belong to this matched cluster pair
                cells_A = idx_A[core_labels_A == cA]
                cells_B = idx_B[core_labels_B == cB]

                if len(cells_A) > 0 and len(cells_B) > 0:
                    # Distribute mass uniformly across the matched block
